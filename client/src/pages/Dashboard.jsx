...
import toast from 'react-hot-toast'; // <--- ADD THIS (to see errors)
...
const Dashboard = () => {
    ...
    const loadAllResumes = async () =>{
        try{
            const {data} = await api.get('/api/resumes/all', {headers : {Authorization : `Bearer ${token}` }})
    ...
    const editTitle = async (event) =>{
        try{
            ...
            setAllResumes(allResumes.map(res => res._id === editResumeId ? data.resume : res));            
            setTitle('')
            seteditResumeId('')
    ...
    const deleteResume = async (resumeId) =>{
        try {
            ...
            const confirm=window.confirm('Are you sure you wanna delete this resume?')
        if(confirm){
    ...
    <div>
            ...
            {allResumes.map((resume, index)=>{
                ...
                return(
                    <button key={index} onClick={()=> navigate(`/app/builder/${resume._id}`)} className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 border group hover:shadow-lg transition-all duration-300 cursor-pointer' style={{background: `linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`, borderColor: baseColor + '40'}}>
                        ...
                        <p className='absolute bottom-1 text-[11px] text-slate-400 group-hover:text_slate-500 transition-all duration-300 px-2 text-center' style={{color:baseColor + '90'}}> 
                            Updated on {new Date(resume.updatedAt).toLocaleDateString()}
                        </p>

                        <div onClick={e=>e.stopPropagation()} className='absolute top-1 right-1 group-hover:flex items-center hidden'>
                            <TrashIcon  onClick={()=>{deleteResume(resume._id)}} className='size-7 p-1.5 hover:bg-white/50 rounded text-slate-700 transition-colours'/>
                            <PencilIcon onClick={()=>{seteditResumeId(resume._id); setTitle(resume.title)}} className='size-7 p-1.5 hover:bg-white/50 rounded text-slate-700 transition-colours'/>
                        </div>
                    </button>
                )
            })}
        ...
        {ShowCreateResume && (
            ...
            <div onClick={e =>e.stopPropagation()}className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <XIcon className='absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colours' onClick={()=>{setShowCreateResume(false); setTitle('')}}/>
            </div>
            </form>
        )}
        ...
        {ShowUploadResume && (
            ...
            <div onClick={e =>e.stopPropagation()}className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <div>
                    <label htmlFor='resume-input' className='block text-sm text-slate-700'>
                        Select resume file
                        <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400 border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-green-500 hover:text-green-700 cursor-pointer transition-colours' >
                        {resume ? (
                            <p className='text-green-700'>{resume.name}</p>
                        ) : (
                            <>
                            <UploadCloud className='size-14 stroe-1'/>
                            <p>Upload Resume</p>
                            </>
                        )}
                        </div>
                ...
                <XIcon className='absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colours' onClick={()=>{setShowUploadResume(false); setTitle('')}}/>
            </div>
            </form>
        )}
         ...
         {editResumeId && (
            ...
            <div onClick={e =>e.stopPropagation()}className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <XIcon className='absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colours' onClick={()=>{seteditResumeId(''); setTitle('')}}/>
            </div>
            </form>
        )}
    </div>
